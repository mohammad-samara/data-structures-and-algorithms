...
const templateWithJQuery = () => {
  ...
  for(let i=0; i<starWarsPeople.length; i++){
      let cloneItem = $('#template').clone();
      cloneItem.removeAttr('id');
      cloneItem.find('h2').html(starWarsPeople[i].name);
      cloneItem.find('h3').html(starWarsPeople[i].height);
      cloneItem.find('p').html(starWarsPeople[i].eye_color);
      cloneItem.appendTo('main');
