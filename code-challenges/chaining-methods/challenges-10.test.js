...
const createServer = () => {
    ...
    app.get('/hello', (request, response) => {
        response.status(200).send("hello user");
    });
    app.get('/aboutme', (request, response) => {
        response.status(200).send("my name is mohammad samara");
    });
    app.get('/favoritefoods', (request, response) => {
        response.status(200).send(["mansaf","burger","maftoul"]);
    });
...
const count = (target, input) => {
    // Solution code here...
    let countArr = input.map(arr =>{return arr.filter(n => n==target).length;});
    let matchedCount = countArr.reduce((acc,n) => {
        acc = acc + n;
        return acc;
    },0);
    return matchedCount;
...
const totalSum = (input) => {
    ...
    let numsArr = input.map(arr =>{
       ...
       return c;
         ...
         let finalSum = numsArr.reduce((acc,el) => {
           acc = acc + el;
           return acc;
       },0);
         return finalSum;
...
const divisibleByFiveTwoToThePower = (input) => {
    // Solution code here...
    let z = input.map(arr =>{
        let y = arr.reduce((acc,el) => {
          if(el%5==0 && (el+el == el*2) && !isNaN(el)){
              acc.push(Math.pow(2, el));
              }
              return acc;
          },[]);
          return(y);
