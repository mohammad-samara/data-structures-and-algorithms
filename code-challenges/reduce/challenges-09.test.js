...
const createServer = () => {
  ...
  app.get('/events', (request, response) => {
    response.status(200).send(getCurrentEvents(request, response));
  })
...
const mapCurrentEvents = () => {
  ...
  //return currentEvents.news.map(n =>new Event(n));
}
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((accomulative,val) => {
    return accomulative+= 1;
  },0)
};
